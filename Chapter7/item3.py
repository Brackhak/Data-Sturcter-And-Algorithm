...
class BST:
    ...
    def power_three(self, node , power, level = 0):
        
        if node != None:
            self.power_three(node.right,power, level + 1)
            if node.data > power :
                node.data *= 3
            print('     ' * level, node)
            self.power_three(node.left,power, level + 1)

T = BST()
inp , power = input("Enter Input : ").split('/')
inp = inp.split(' ')
for i in inp:
    root = T.insert(int(i))
T.printTree(root)
print("--------------------------------------------------")
T.power_three(root,int(power))